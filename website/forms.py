    ...
    def clean_title(self):
        ...
        if len(title) < 12:
            raise forms.ValidationError(
                "Title should be longer than 12 characters")
    ...
    def clean_image(self):
        ...
        if (image is not None):
            if (image.size > settings.MAXIMUM_FILE_SIZE):
                raise forms.ValidationError(
                    'File size needs to be under {0} MB.'.format(
        ...
        if category == '12':
            if FossCategory.objects.filter(id=category).exists():
                ...
                self.fields['tutorial'] = forms.CharField(
                    widget=forms.Select(choices=tutorial_choices))
                if SubFossCategory.objects.filter(
                        name=selecttutorial).exists():

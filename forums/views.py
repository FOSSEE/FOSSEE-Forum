    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                """
                Please confirm your registration by clicking on the activation
                link which has been sent to your registered email id.
                """)
            return render(request, 'forums/templates/message.html')
        context = {'form': form}
        return render_to_response('forums/templates/user-register.html',
                                  context,
...
def confirm(request, confirmation_code, username):
    try:
        ...
        messages.success(request,
                         "Your account not activated!. Please try again!")
        return HttpResponseRedirect('/')
...
def account_profile(request, username):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # if 'picture-clear' in request.POST and
            # request.POST['picture-clear']:
            ...
            """if 'picture' in request.FILES:
                ...
                if ext != '.pdf' and ext != '':
                    im = Image.open(settings.MEDIA_ROOT +
                                    str(form_data.picture))
                    ...
                    if mimeType == 'JPG':
                        mimeType = 'JPEG'
                    im.save(settings.MEDIA_ROOT + "user/" + str(user.id) +
                            "/" + str(user.id) + "-thumb." + ext, mimeType)
                    form_data.thumb = 'user/' + str(user.id)+ '/' +
                                      str(user.id) + '-thumb.' + ext
                    form_data.save()"""
            messages.success(request, "Your profile has been updated!")
            return HttpResponseRedirect("/accounts/view-profile/{0}"
                                        .format(user.id))
...
def send_registration_confirmation(user):
    ...
    You may activate your account by clicking on this
    ...
    email = EmailMultiAlternatives(
        ...
        headers={'Reply-To': 'forums@fossee.in',
...
#         else:
#             return render_to_response("forums/templates/update-password.html",
#                                       context)
#     else:
#         form = UserLoginForm()
#         context['form'] = form
